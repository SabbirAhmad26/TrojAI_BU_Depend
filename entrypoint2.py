...
full_path = os.path.join(current_directory, 'trojai-example')
...
if __name__ == "__main__":
    ...
    subparser = parser.add_subparsers(dest='cmd', required=not DEBUG)
    ...
    # Set 'configure' as the default subcommand if none is provided
    #if not args.command:
    #    print("No subcommand provided. Defaulting to 'configure' mode.")
    #    args = configure_parser.parse_args()  # Parse again with configure parser
    ...
    # Checks if new mode of operation is being used, or is this legacy
        ...
        # Set default arguments for configure mode
        default_args = [
            'configure',
            '--scratch_dirpath', './trojai-example/scratch',  # Default path for scratch
            '--configure_models_dirpath', './trojai-example/models',  # Default path for models
            '--metaparameters_filepath', './trojai-example/metaparameters.json',  # Default path for metaparameters
            '--schema_filepath', './trojai-example/metaparameters_schema.json',  # Default path for schema
            '--learned_parameters_dirpath', './trojai-example/learned_parameters',  # Default path for learned parameters
        ]
        infer_args = [
            'infer',
            '--model_filepath', './trojai-example/model/rl-colorful-memory-sep2024-example/model.pt', #path for model to infer on
            '--result_filepath', './trojai-example/output.txt', #path for output
            '--scratch_dirpath', './trojai-example/scratch',  #  path for scratch
            '--round_training_dataset_dirpath', './trojai-example/models', #path for models
            '--metaparameters_filepath', './trojai-example/metaparameters.json',  # Dfult path for metaparameters
            '--schema_filepath', './trojai-example/metaparameters_schema.json',  #  path for schema
            '--learned_parameters_dirpath', './trojai-example/learned_parameters',  #  path for learned parameters
        ]
        if DEBUG:
        #'infer' mode if no specific command is provided
            args = parser.parse_args(default_args)  # Add 'infer' as the command
            args.func(args)
        else:
